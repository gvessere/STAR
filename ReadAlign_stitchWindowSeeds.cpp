        ...
        for (uint iS2=0;iS2<=iS1;iS2++) {
            ...
            if (iS2<iS1) {
                ...
                if ( WA[iW][iS1][WA_rStart]>0 \
                     && extendAlign(R, Q, G, WA[iW][iS1][WA_rStart]-1, WA[iW][iS1][WA_gStart]-1, -1, -1, min(WA[iW][iS1][WA_rStart], WA[iW][iS1][WA_gStart] - P->chrStart[WC[iW][WC_Str]]), 100000, 0, P->outFilterMismatchNmax, P->outFilterMismatchNoverLmax, &trA1) ) {//if could extend
...
//                     scoreSeedBestMM[iS1]=trA1.nMM;                    
    ...
    for (uint iS1=0;iS1<nWA[iW];iS1++) {//find the best alignment
       ...
       if ( tR2 < Lread-1 \
            && extendAlign(R, Q, G, tR2, tG2, +1, +1, min(Lread-tR2,P->chrStart[WC[iW][WC_Str]+1]-tG2-1), 100000, scoreSeedBestMM[iS1], P->outFilterMismatchNmax, P->outFilterMismatchNoverLmax, &trA1) ) {//if could extend
...
//                 cout <<"BUG"<<endl;
        ...
        {//extend to the left
            ...
            if ( trA.exons[0][EX_R]>0 \
                 && extendAlign(R, Q, G, trA.exons[0][EX_R]-1, trA.exons[0][EX_G]-1, -1, -1, min(trA.exons[0][EX_R], trA.exons[0][EX_G] - P->chrStart[WC[iW][WC_Str]]), 100000, 0, P->outFilterMismatchNmax, P->outFilterMismatchNoverLmax, &trA1) ) {//if could extend
